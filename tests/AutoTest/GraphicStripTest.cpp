...
void GraphicStripTest::drawTest()
{
	...
	doc->addObject(new PhStripText(5000, doc->peoples().first(), 10000, 0.25f, "Hello", 0.25f));
	doc->addObject(new PhStripCut(5400, PhStripCut::CrossFade));
	doc->addObject(new PhStripDetect(PhStripDetect::Off, 5000, doc->peoples().first(), 10000, 0.25f));
	...
	QImage resultImage(view.renderPixmap(700, 240).toImage());
	...
	QVERIFY(result < 700 * 240 / 4); // accept a difference of 1 per 4 pixels
}
